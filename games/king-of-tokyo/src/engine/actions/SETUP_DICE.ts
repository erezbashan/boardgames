import { KotState, PendingAction } from '../types';
import { addLog } from '../utils';

export function handleSetupDice(st: KotState, action: PendingAction, pId: string) {
  const diceCount = 6;
  const DICE_FACES = ['1', '2', '3', 'Energy', 'Heart', 'Smash'];
  st.dice = Array.from({ length: diceCount }).map((_, i) => ({ id: `d${i}`, value: DICE_FACES[Math.floor(Math.random() * DICE_FACES.length)] as any, kept: false }));
  const extraRerolls = st.players[pId].stats?.extraRerolls || 0;
  st.maxRolls = 3 + extraRerolls;
  st.rollCount = st.maxRolls;
  addLog(st, action, `${st.dice.length} dice are ready for up to ${st.rollCount} rolls.`);
}
