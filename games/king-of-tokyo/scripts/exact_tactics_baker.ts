...
const VTable = require('../src/bots/exactConfig.json');
...
function getV(mVp: number, mHlt: number, oVp: number, oHlt: number, tok: number): number {
    ...
    let key = getStateKey(mVp, mHlt, oVp, oHlt, tok);
...
for (const vps of [true, false]) {
    for (const att of [true, false]) {
        for (const hlt of [true, false]) {
            DICE_COMBOS.push({ VPS: vps, ATT: att, HLT: hlt, ENR: false });
        }
    }
}

const FACES = ['1', '2', '3', 'Heart', 'Smash', 'Energy'];
function randomRoll() { return FACES[Math.floor(Math.random() * FACES.length)]; }

function simulateTurn(mVp: number, mHlt: number, oVp: number, oHlt: number, tok: number, tactic: any): number {
    let start_mVp = mVp;
    if (tok === 1) {
...
console.log("Baking Tactics for all 80,000 states (200 sims each)...");
const BakedTactics: Record<string, any> = {};
        ...
        for (let oVp = 0; oVp < 20; oVp++) {
            for (let oHlt = 1; oHlt <= 10; oHlt++) {
                for (let tok of [0, 1]) {
                    ...
                    for (let tactic of DICE_COMBOS) {
                        let exp = 0;
                        for (let i = 0; i < 200; i++) {
                            exp += simulateTurn(mVp, mHlt, oVp, oHlt, tok, tactic);
                        }
                        exp /= 200.0;
                        if (exp > bestExp) {
                            bestExp = exp;
                    ...
                    BakedTactics[getStateKey(mVp, mHlt, oVp, oHlt, tok)] = bestTactic;
                    
                    statesProcessed++;
                    if (statesProcessed % 10000 === 0) {
                        console.log(`Processed ${statesProcessed} / 80000 states...`);
                    }
                }
            }
        }
...
console.log("Saving BakedTactics to exactTactics.json...");
fs.writeFileSync(path.join(__dirname, '../src/bots/exactTactics.json'), JSON.stringify(BakedTactics));
console.log("Done!");
