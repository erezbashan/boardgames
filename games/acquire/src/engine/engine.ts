...
export function buyStock(state: AcquireState, playerId: string, corpName: Corporation): AcquireState {
  ...
  for (let i = newLogs.length - 1; i >= Math.max(0, newLogs.length - 10); i--) {
    ...
    if (!foundBuyLog) {
      const match = log.match(new RegExp(`^${player.name} bought (\\d+) shares? of ${corpName}\\.?import type { BoardCell, Corporation, CorporationState, AcquireState, AcquirePlayer, Tile, TileId } from './types';
